...
public class MetricController implements Controller{
    ...
    //This is to calculate the average number of bins owned by each resident, which can reflect the average garbage generation level and classification degree of residents.
    ...
    //calAverageFillLevel is to calculate the average fill level of all bins, which can reflect the degree of garbage retention in the city
    public double calAverageFillLevel(){
        for(int i = 0; i < bins.getBins().size(); i++){
            this.averageFillLevel = (this.averageFillLevel * i + bins.getBins().get(i).getFillLevel() )/ (i+1); // averageFillLevel * i is the sum of levels for all other bins + current  bin / number of bins.
        }
        return this.averageFillLevel;
    }

    //recyclageLevel is the recycling rate of garbage, which can reflect the efficiency of urban residents' activities on garbage recycling. calcul by FillLevel Recy / FillLevel for all
