...
public class Metric {
    ...
    //This function is to calculate the average number of bins owned by the resident, which can reflect the average garbage generation level and classification degree of the person.

    public double calAverageFillLevelRes(){
        double averageFillLevelRes = 0;

        for(int i = 0; i < binsRes.size(); i++){
            averageFillLevelRes = (averageFillLevelRes * i + binsRes.get(i).getFillLevel() )/ (i+1); // averageFillLevel * i is the sum of levels for all other bins + current  bin / number of bins.
        }
        return averageFillLevelRes;
    }

    //This ratio is the comparison between the public garbage collection rate and the individual
    // residents' garbage collection rate. The higher the ratio, the greater the contribution of residents to recycling garbage.
    ...
    //calculate the current utilisation of fillLevel for all bins from this resident
